...
/**
 *  Can be synthesized if needed
 *  into a Pure Shopify object
 */
const is_flexible = d => {
    if (_.is.String(d)) {
        return true
    } else if (_.is.Integer(d)) {
        return true
    } else if (_.is.Integer(d.id)) {
        return true
    } else {
        return false
    }
...
is_flexible.method = "_.is.shopify.flexible"

/**
 *  Is JSON and has "id".
 */
const is_pure = d => _.is.JSON(d) && _.is.Integer(d.id)

is_pure.method = "_.is.shopify.pure"

/**
 *  Monkeypatch
 */
_.is.shopify = is_pure
_.is.shopify.flexible = is_flexible
